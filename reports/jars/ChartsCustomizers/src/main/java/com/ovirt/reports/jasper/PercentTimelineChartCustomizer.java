...
public class PercentTimelineChartCustomizer implements JRChartCustomizer {
	...
	public void customize(JFreeChart chart, JRChart jasperChart) {
	        ...
	        for (int i = 0; i < chartLegend.getItemCount(); i++) {
	           ...
	           if (label.length() > 18)
	           {
	        	   if (label.contains("(Removed".subSequence(0, 7)))
	        	   {
				   	        	   if (label.length() - 23 > 18)
									{
										label = label.substring(0,10).concat("...").concat(label.substring(label.length() - 28, label.length()));
									}
	        	   }
	        	   else
	        	   {
